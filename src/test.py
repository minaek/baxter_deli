...
def arm_give_cookie():
    ...
    #lifts arm above cookies
left.move_to_joint_positions({'left_w0': -1.2766555107176356, 'left_w1': 0.5311408478053246, 'left_w2': 3.0480198255283173, 'left_e0': 0.37045636027432743, 'left_e1': 1.2659176452024377, 'left_s0': -0.6082233823965666, 'left_s1': -0.3604854851530722})


        #lowers to grab cookie
left.move_to_joint_positions({'left_w0': -0.8678496307461728, 'left_w1': -0.05714078434873166, 'left_w2': 2.489267323541055, 'left_e0': 0.27803401780423126, 'left_e1': 1.4388739790365181, 'left_s0': -0.7597039852002512, 'left_s1': -0.3869466537440956})
...
def arm_take_pos_l():
    print "<hand item to Baxter's left arm>"

    #open gripper
    leftg.open()

    #reach arm out to grab item
    left.move_to_joint_positions({'left_w0': -0.7597039852002512, 'left_w1': 0.540344732532637, 'left_w2': 2.090048823493876, 'left_e0': 0.4249126782442596, 'left_e1': 0.2715145994557182, 'left_s0': -0.9495341077010712, 'left_s1': 0.023009711818281205})

    #close gripper
    leftg.close()

def arm_take_pos_r():
    print "<hand item to Baxter's right arm>"

    #open gripper
    rightg.open()

    #reach arm out 
    right.move_to_joint_positions({'right_s0': 0.7409127205486549, 'right_s1': -0.0724805922275858, 'right_w0': 0.4084223847744914, 'right_w1': 0.4678641403050512, 'right_w2': -1.021247709534714, 'right_e0': -0.47016511148687934, 'right_e1': 0.46978161628990794})
